...
import illustrate11 from "../assets/illustrate11.svg";
...
const HospitalLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const res = await axios.post("http://localhost:8000/hospitallogin",dataToSend);
        if(res.status==200) {
            ...
            setTimeout(() => {
                history(`/HospitalOTP?email=${dataToSend.email}`);
      ...
      <div className="px-10 py-8 bg-slate-100 dark:bg-slate-800 shadow-lg shadow-gray-600">
        ...
        <div className="head-banner flex flex-col md:flex-row p-5 w-full h-auto bg-violet-800 rounded-2xl items-center justify-center">
            ...
            <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
              <div>
                <input
                  ...
                  placeholder="Email ID" value={hospitalData.email} onChange={handleInputChange}
                />
              </div>
              <div>
                <input
                  ...
                  required
                  className="mt-1 p-3 block w-full border-2 border-violet-600 rounded-md placeholder-violet-600 text-violet-600 outline-none font-bold"
                  placeholder="Password" value={hospitalData.password} onChange={handleInputChange}
                />
              </div>
              <div>
                <Link
          ...
          <div className="w-full md:w-[50%] flex flex-col items-center">
            <img src={illustrate11} className="w-[500px]" />
          </div>
        </div>
      </div>
      <Footer />
